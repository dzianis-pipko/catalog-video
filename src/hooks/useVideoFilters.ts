...
export const useVideoFilters = (videos: IVideo[], initialFilters?: Partial<VideoFilters>) => {
  ...
  // Инициализация фильтров из URL или из начальных значений
   const [filters, setFilters] = useState<VideoFilters>(() => {
      ...
      const durationFilter = urlDuration ? (urlDuration as TDurationFilter) : initialFilters?.durationFilter || 'all';
      const sortBy = urlSort ? (urlSort as TSortOption) : initialFilters?.sortBy || 'date';
